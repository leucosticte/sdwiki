...
class SkinTemplate extends Skin {
	...
	protected function buildContentNavigationUrls() {
		...
		if ( $title->canExist() ) {
			...
			$isHistory = false;
			$newTitle = $title;
			#echo $newTitle->getFullText ();
			
	        if ( $newTitle->getNamespace() === 130 ) { // If so, it must be either History:User: or History:User talk:, etc.
				$isHistory = true;
				$newTitle = Title::newFromText( $title->getText() ); // Strip it till all we have is User:, User talk:, etc.
				#echo $newTitle->getFullText ();
			}
			#die();
			#echo ('foo' );
			...
			if ( !$newTitle->isTalkPage() ) {
				...
				#echo ('boo' );
				$subjectTitle = $newTitle->getSubjectPage();
				if ( $isHistory ) {
					$isHistory = false;
					$isHistoryTalk = true;
				}
			...
			#die ( 'bar' );
			$talkTitle = $subjectTitle->getTalkPage();
			$historyTitle = Title::newFromText( 'History:' . $subjectTitle->getFullText () );
			$historyTalkTitle = Title::newFromText( 'History:' . $talkTitle->getFullText () );
			$subjectText = $subjectTitle->getFullText();
			$talkText = $talkTitle->getFullText();
			$historySubjectText = $historyTitle->getFullText();
			$historyTalkText = $historyTalkTitle->getFullText();
			$historyId = $historyTitle->getNamespaceKey( '' );
			$historyTalkId = $historyTalkTitle->getNamespaceKey( '' );
			$subjectPage = $subjectTitle;
			$talkPage =$talkTitle;
			...
			$historyMsg = [ "nstab-$historyId" ];
			#$historyTalkMsg = [ "nstab-$historyTalkId" ];
			$historyTalkMsg = "nstab-historytalk";
			#$historyTalkMsg = 'Discussion history';
			if ( $subjectPage->isMainPage() ) {
				array_unshift( $subjectMsg, 'mainpage-nstab' );
			}
			$content_navigation['namespaces'][$subjectId] = $this->tabAction(
				$subjectPage, $subjectMsg, !$isTalk && !$isHistory && !$isHistoryTalk && 
					!$preventActiveTabs, '', $userCanRead
			);
			$content_navigation['namespaces'][$subjectId]['context'] = 'subject';
			$content_navigation['namespaces'][$historyId] = $this->tabAction(
				$historyTitle, $historyMsg, $isHistory && !$preventActiveTabs, '', $userCanRead
			);
			$content_navigation['namespaces'][$historyId]['context'] = 'subject';
			...
			$content_navigation['namespaces']['historytalk'] = $this->tabAction(
				$historyTalkTitle, $historyTalkMsg, $isHistoryTalk &&
				!$preventActiveTabs, '', $userCanRead
			);
			$content_navigation['namespaces']['historytalk']['context'] = 'subject';
