...
global $wgSdUser;			# SD
global $wgSdContentmodel;	# SD
global $wgSdContentformat;	# SD
global $wgSdTags;			# SD
global $wgSdRevRemoteId;	# SD
...
class ApiEditPage extends ApiBase {
	public function execute() {
		...
		if ( isset( $params['sdtimestamp'] ) ) {						# SD
			$wgSdTimestamp = $params['sdtimestamp'];					# SD
		}																# SD
		if ( isset( $params['sduser'] ) ) {								# SD
			$wgSdUser = $params['sduser'];								# SD
		}																# SD
		if ( isset( $params['sdcontentmodel'] ) ) {						# SD
			$wgSdContentmodel = $params['sdcontentmodel'];				# SD
		}																# SD
		if ( isset( $params['sdcontentformat'] ) ) {					# SD
			$wgSdContentformat = $params['sdcontentformat'];			# SD
		}																# SD
		if ( isset( $params['sdtags'] ) ) {								# SD
			$wgSdTags = $params['sdtags'];								# SD
		}																# SD
		if ( isset( $params['sdrevremoteid'] ) ) {						# SD
			if ( isset( $params['sdsaverevremoteid'] ) ) {				# SD
				$wgSdRevRemoteId = $params['sdrevremoteid'];			# SD
			}															# SD
			$dbw = wfGetDB( DB_MASTER );								# SD
			$field = $dbw->selectField(									# SD
				'revision',												# SD
				'rev_remote_id',										# SD
				array( 'rev_remote_id' => $params['sdrevremoteid'] )	# SD
			);															# SD
	...
	public function getAllowedParams() {
		return [
			...
			'sdsaverevremoteid' => [					# SD
				ApiBase::PARAM_TYPE => 'boolean',		# SD
				ApiBase::PARAM_DFLT => false			# SD
			],
		];
	}
